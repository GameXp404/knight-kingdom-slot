...
$src = 'C:\Users\USER\AppData\Local\Temp\dodol_temp.png'
...
for ($i = 0; $i -lt $bytes.Length; $i += 4) {
    ...
    $minC = [Math]::Min($r, [Math]::Min($g2, $b))
    if ($minC -gt $threshHigh) {
        $bytes[$i+3] = 0; $changed++
    } elseif ($minC -gt $threshFade) {
        $fade = ($threshHigh - $minC) / ($threshHigh - $threshFade)
        $bytes[$i+3] = [byte]([int]([byte]$bytes[$i+3] * $fade))
    }
}
[System.Runtime.InteropServices.Marshal]::Copy($bytes, 0, $data.Scan0, $bytes.Length)

# Detect hollow center (now scanning for transparent area)
...
Write-Host ("Image: {0}x{1} cleared={2}%" -f $w, $h, $pct)
...
Write-Host ("Sprite border = ({0}, {1}, {2}, {3})" -f $lBorder, $bBorder, $rBorder, $tBorder)
